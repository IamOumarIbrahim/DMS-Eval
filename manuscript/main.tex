...
Under our single-frame annotation protocol, each sampled image contains at most one visual warning cue annotation (0 or 1 bounding box per image). In particular, \texttt{yawning} and \texttt{hand\_over\_mouth} are mutually exclusive; if a driver yawns while a hand visibly occludes the mouth, the instance is uniquely labeled as \texttt{hand\_over\_mouth}, ensuring non-overlapping ground-truth targets.

All models are evaluated under a strict 8/3/3 subject-disjoint split ($S_{\text{train}} \cap S_{\text{val}} = \emptyset,\; S_{\text{train}} \cap S_{\text{test}} = \emptyset,\; S_{\text{val}} \cap S_{\text{test}} = \emptyset$) selected via exhaustive combinatorial search across 60,060 candidate assignments ($\le 5.48\%$ maximum distribution divergence). Training is executed under identical hardware (NVIDIA RTX 4060), batch size 1, 220 epochs, FP16 AMP, with optimal thresholds selected strictly on the validation split prior to isolated test evaluation.
...
Random train/test partitioning across the same human subjects allows models to memorize biometric features rather than visual cues, inflating accuracy \cite{rong2021artificial}. DMS-Eval enforces subject-disjoint partitioning ($S_{\text{train}} \cap S_{\text{test}} = \emptyset$).
