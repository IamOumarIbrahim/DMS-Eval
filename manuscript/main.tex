...
\item \textbf{Unified Precision--Efficiency Protocol:} Identical hardware (RTX 4060), physical batch size~1 with gradient accumulation across 32 steps, 220 epochs, FP16 AMP, with detection accuracy and hardware-synchronized latency ($p50$/$p95$/$p99$) reported jointly.
...
\item \textbf{Batch Size \& Gradient Dynamics}: Physical batch size is strictly locked to 1 to emulate real-time edge frame ingestion, paired with gradient accumulation across 32 steps (nominal batch size 32, \texttt{nbs=32}) to stabilize Batch Normalization running statistics and optimization trajectories on GPU hardware.
