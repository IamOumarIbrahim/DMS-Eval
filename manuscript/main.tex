...
\caption{DMS-Eval lifecycle: $640\times640$ extraction and manual annotation; deterministic 8/3/3 subject-disjoint splitting; RTX 4060 training with batch 8, accumulation 4, 220 epochs, seeds 13/37/73, no early stopping, and pinned recipes; validation-only freezing; and one environment-gated test pass per model--seed run.}
\label{fig_pipeline}
\end{figure*}
...
All models use the same training split/classes, $640\times640$ input, physical batch 8, fixed four-step accumulation from the first batch, 220 epochs, disabled early stopping, FP16 AMP, the three predeclared seeds 13/37/73, and the RTX 4060. Every image is retained with \texttt{drop\_last=false}; the final incomplete accumulation window is sample-correct for each backend's mean- or sum-reduced loss. Every run is retained, and model results use mean $\pm$ sample standard deviation without best-run selection. Validation may retain and rank checkpoints but never reloads or otherwise changes training state. D-FINE's epoch-148 augmentation/EMA transition is predefined and continues the current model and optimizer state.

Architecture-specific optimizer, learning-rate, scheduler, weight-decay, and augmentation settings follow pinned official recipes. Ultralytics 8.4.123 retains \texttt{optimizer=auto} (expected MuSGD for the planned iteration count), its default learning rate and linear schedule; D-FINE-N retains AdamW with base/backbone learning rates $0.0008/0.0004$ and weight decay $0.0001$. The closed adaptation list is: DMS-Eval dataset/classes, $640\times640$ input, batch 8, accumulation 4, 220 epochs, seeds 13/37/73, and disabled early stopping. No model-specific tuning is performed. Equal epochs therefore mean equal data exposure, not equal FLOPs, wall time, or capacity.
...
\end{equation}
where $P(\tau) = \frac{\text{TP}(\tau)}{\text{TP}(\tau) + \text{FP}(\tau)}$ and $R(\tau) = \frac{\text{TP}(\tau)}{\text{TP}(\tau) + \text{FN}(\tau)}$ under same-class greedy one-to-one IoU $\ge 0.50$ matching. Each run's optimal validation checkpoint and threshold $\tau^*$ are permanently frozen. Each of the nine model--seed manifests receives exactly one Test-split pass without further tuning.
...
No result is available before the authorized frozen runs. Table~\ref{tab_results} pre-registers the system-level quality--efficiency comparison, and Table~\ref{tab_per_class} pre-registers per-class AP. The aggregate pipeline requires all nine results and reports mean $\pm$ sample standard deviation; no placeholder is presented as evidence.

\IfFileExists{figures/accuracy_latency.png}{%
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{accuracy_latency.png}
\caption{Protected-test quality versus tensor-to-final-detections median latency (mean $\pm$ sample standard deviation over three seeds).}
\label{fig_accuracy_latency}
\end{figure}}{}

\begin{table*}[htbp]
\caption{Pre-Registered Comparison (RTX 4060, Batch 1, CUDA AMP FP16; Mean $\pm$ Sample SD; All Fields Pending)}
\begin{center}
\resizebox{\textwidth}{!}{
...
\begin{table}[htbp]
\caption{Per-Class AP@0.5:0.95 Framework (Mean $\pm$ Sample SD on Test Split)}
\begin{center}
\resizebox{\columnwidth}{!}{
\begin{tabular}{lcccc}
\toprule
...
The protected-pass qualitative/error generator uses five outcome-defined categories fixed before training:
\begin{itemize}
\item correct detections and false-positive detections on negative frames;
\item false-negative, class-confusion, and localization-error images.
\end{itemize}
Three candidates per category are ranked deterministically during each test pass. The publication contact sheet uses predeclared reference seed 13, while error counts use all seeds; no post-hoc example search rereads the test set.

\subsection{Threats to Validity}
\begin{itemize}
\item \textbf{Internal Validity:} Shared data exposure, fixed accumulation, sample-correct remainders, validation-only selection without trajectory intervention, pinned official recipes, and protected testing limit avoidable asymmetry but do not isolate architecture. Three seeds quantify limited, not exhaustive, stochastic variance.
...
\section{Conclusion}
This paper defines \textbf{DMS-Eval}, a reproducible benchmark for three lightweight in-cabin detector systems. Its 15,723-frame dataset, four warning cues, negative-frame emphasis, subject-disjoint split, closed training adaptations, shared evaluator, dual runtime boundaries, three-seed reporting, and protected test procedure address key leakage and comparability risks. The research question cannot be answered until the frozen runs produce evidence. Even then, conclusions concern the configured systems, not architecture-only causation or broad statistical superiority.
