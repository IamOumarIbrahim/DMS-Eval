...
\item \textbf{Unified Precision--Efficiency Protocol:} Identical hardware (RTX 4060), physical training batch 8 with gradient accumulation over 4 steps (effective batch 32), 220 epochs, FP16 AMP, with detection accuracy and hardware-synchronized batch-1 latency ($p50$/$p95$/$p99$) reported jointly.
...
As documented in Table~\ref{tab_splits} and Fig.~\ref{fig_distributions_and_splits}(c), the maximum relative deviation across all splits and classes is restricted to $\le 5.4812\%$, ensuring near-identical distribution fidelity across partitions.
...
\item \textbf{Batch Size \& Gradient Dynamics}: Physical mini-batch size is locked to 8 (fitting the RTX 4060 8~GB VRAM budget at $640\times 640$), paired with gradient accumulation across 4 steps (nominal batch size 32, \texttt{nbs=32}) to stabilize Batch Normalization running statistics and Hungarian bipartite matching.
...
Optimization hyperparameters (SGD with momentum for CNN detectors vs. AdamW for D-FINE-N, alongside native learning rate schedules and augmentations) follow each architecture's authoritative repository recipes to prevent artificial distortion from mismatched optimization regimes. To accommodate cross-paradigm dynamics between convolutional networks and transformer set-prediction decoders, an effective batch size of 32 stabilizes gradient estimates, while model checkpoints are selected via peak validation $\text{mAP}@0.5:0.95$ rather than terminal epoch weights, mitigating potential late-schedule overfitting on convolutional models while allowing transformers sufficient trajectory to converge.
...
In accordance with standard edge ML benchmarking guidelines \cite{reddi2020mlperf,shafi2021demystifying,Jacob_2018_CVPR}, inference speed is measured under native PyTorch in FP16 on the RTX 4060 GPU at batch size 1. Following 10 untimed warm-up passes, inference latency is measured across all test frames using hardware-synchronized CUDA events (\texttt{torch.cuda.Event}). We report median latency ($p50$), 95th percentile ($p95$), 99th percentile ($p99$), and sustained throughput (FPS). Peak memory allocation is measured via \texttt{torch.cuda.max\_memory\_allocated()} to evaluate edge SRAM/VRAM viability. To evaluate false-positive suppression on the $12,722$ negative background frames ($N_{\text{neg}}$), we formalize the Background False Alarm Rate:
\begin{equation}
\text{FAR} = \frac{\text{FP}_{\text{neg}}}{N_{\text{neg}}} \times 100\%
\end{equation}
where $\text{FP}_{\text{neg}}$ represents false detections triggered on un-cued driving frames.
...
\toprule
\textbf{Model} & \textbf{Params} & \textbf{FLOPs} & \textbf{VRAM (MB)} & \textbf{Lat. $p50$} & \textbf{Lat. $p95$} & \textbf{Lat. $p99$} & \textbf{FPS} & \textbf{FAR (\%)} & \textbf{mAP@0.5:0.95} & \textbf{mAP@0.5} & \textbf{Prec.} & \textbf{Recall} & \textbf{F1} \\
\midrule
YOLO11n & 2.6M & 6.5G & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] \\
YOLO26n & 2.4M & 5.8G & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] \\
D-FINE-N & 3.8M & 8.4G & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] & [TO\_BE\_FILLED] \\
\bottomrule
\end{tabular}
}
\label{tab_results}
...
Empirical error inspection across the evaluation splits reveals three primary challenging edge-case failure modes:
\begin{itemize}
\item \textbf{High-Frequency Motion Blur:} Rapid hand gestures during steering transitions cause boundary blurring, leading to missed detections or localization jitter.
\item \textbf{Extreme Ambient Illumination Transitions:} Direct solar glare and sharp cabin shadows distort facial contrast, occasionally inducing false-positive \texttt{hand\_over\_mouth} triggers on empty chin-rest postures.
\item \textbf{Oblique Head Yaw and Re-identification Occlusion:} Severe lateral driver head turns truncate facial landmarks, challenging detectors lacking temporal tracking context.
\end{itemize}

\subsection{Threats to Validity}
\begin{itemize}
\item \textbf{Internal Validity:} Fixed training seeds and locked optimizer configurations guarantee fair comparisons. While single-seed execution (seed 13) was adopted due to compute budget ceilings, deterministic CUDA seeding and validation-only threshold calibration ($\tau^* = \arg\max_{\tau} F_1(\tau; S_{\text{val}})$) prevent optimistic test leakage. Checkpointing at peak validation $\text{mAP}@0.5:0.95$ guards against late-epoch variance.
...
This benchmark builds upon the open-source architectures and tooling developed by the teams behind Ultralytics YOLO \cite{yolo11_ultralytics,yolo26_ultralytics}, D-FINE \cite{peng2024dfine}, and Label Studio \cite{label_studio,label_studio_converter}. The authors also acknowledge [TO\_BE\_FILLED: Grant / Compute Infrastructure Support] for supporting this research.
