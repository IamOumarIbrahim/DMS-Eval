...
| **M1.5** | **Export & Assemble Master COCO JSON** | Export finalized annotations from Label Studio. Merge and format into single authoritative COCO format ground truth saved at `dataset/annotations.json`. Verify image paths, IDs, categories (1–4), and bounding box coordinates $[x, y, w, h]$. | Label Studio Export | [`dataset/annotations.json`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/dataset/annotations.json) | ⏳ Pending | Valid COCO JSON schema. Bounding boxes strictly within $640 \times 640$ image dimensions. Exactly 4 category definitions. |
