...
| **M1.3** | **Direct Manual Human Annotation (100% Frames)** | Human expert manually inspects and annotates every single sampled frame (including zero-cue frames) in Label Studio. Enforce bounding box extents: `yawning` (mouth only), `hand_over_mouth` (full head), `drinking` (face+bottle), `phone_use` (hand+phone at ear). | Label Studio UI; [`docs/annotation-protocol.md`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/docs/annotation-protocol.md); [`docs/manual-annotation-guide.pdf`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/docs/manual-annotation-guide.pdf) | Authoritative annotations in Label Studio | 🔄 In Progress | 100% of frames inspected. No extrapolation of hidden anatomy. Max 1 annotation per image (0 or 1 box); `yawning` and `hand_over_mouth` are mutually exclusive (prioritize `hand_over_mouth` if hand covers mouth). |
...
| **M1.5** | **Export & Assemble Master COCO JSON** | Export finalized annotations from Label Studio via [`label-studio-converter`](https://github.com/HumanSignal/label-studio-converter) and execute [`scripts/assemble_master_coco.py`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/scripts/assemble_master_coco.py). Assemble canonical master COCO ground truth at [`dataset/annotations.json`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/dataset/annotations.json) with relative image paths, 1-indexed category IDs (1–4), and validated $640 \times 640$ bounding boxes. | Label Studio Export; [`scripts/assemble_master_coco.py`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/scripts/assemble_master_coco.py) | [`dataset/annotations.json`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/dataset/annotations.json) | 🧊 Frozen | Valid COCO JSON schema (15,723 images, 3,001 bboxes). Bounding boxes strictly within $640 \times 640$. Exactly 4 category definitions. |
...
| Step ID | Task / Operation | Detailed Execution Instructions | Input / Dependencies | Output / Artifacts | Status | Quality Control & Verification Criteria |
| :--- | :--- | :--- | :--- | :--- | :---: | :--- |
| **M2.1** | **Extract Per-Subject Cue Counts** | Parse master `annotations.json` to compute exact frame-level occurrence frequencies for each of the 4 target warning cues per subject ($S_1, \dots, S_{14}$) via [`scripts/balance_splits.py`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/scripts/balance_splits.py). | [`dataset/annotations.json`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/dataset/annotations.json) | Per-subject cue distribution matrix in [`dataset/split_selection_report.json`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/dataset/split_selection_report.json) | 🧊 Frozen | Frame counts tabulated. All 14 subjects profiled. |
| **M2.2** | **Formalize 8/3/3 Subject Assignment Method** | Implement and run the deterministic exhaustive search algorithm in [`scripts/balance_splits.py`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/scripts/balance_splits.py) evaluating all $\binom{14}{8} \times \binom{6}{3} = 3003 \times 20 = 60,060$ valid 8/3/3 subject partitions. Select candidate minimizing worst absolute relative deviation and RMSE across positive-frame rate and 4 cue proportions against global distribution. | Master `annotations.json`; [`scripts/balance_splits.py`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/scripts/balance_splits.py) | Partition scoring report [`dataset/split_selection_report.json`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/dataset/split_selection_report.json) | 🧊 Frozen | Algorithm guarantees all 4 target cues are present in Train, Val, and Test splits. Deterministic, fully reproducible selection. |
| **M2.3** | **Freeze & Validate `splits.json`** | Write optimal subject assignment into [`dataset/splits.json`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/dataset/splits.json) (`train`: 8 subjects, `validation`: 3 subjects, `test`: 3 subjects). Permanently freeze this file. | Selected subject IDs | [`dataset/splits.json`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/dataset/splits.json) | 🧊 Frozen | Strict disjointness: $\text{Train} \cap \text{Val} = \emptyset$, $\text{Train} \cap \text{Test} = \emptyset$, $\text{Val} \cap \text{Test} = \emptyset$. Total subjects = 8 + 3 + 3 = 14. Maximum relative deviation $\le 5.48\%$. |
