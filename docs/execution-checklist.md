...
| **M1.5** | **Export & Assemble Master COCO JSON** | Export finalized annotations from Label Studio via [`label-studio-converter`](https://github.com/HumanSignal/label-studio-converter). Merge and format into single authoritative COCO format ground truth saved at `dataset/annotations.json`. Verify image paths, IDs, categories (1–4), and bounding box coordinates $[x, y, w, h]$. | Label Studio Export; [`label-studio-converter`](https://github.com/HumanSignal/label-studio-converter) | [`dataset/annotations.json`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/dataset/annotations.json) | ⏳ Pending | Valid COCO JSON schema. Bounding boxes strictly within $640 \times 640$ image dimensions. Exactly 4 category definitions. |
...
| Step ID | Task / Operation | Detailed Execution Instructions | Input / Dependencies | Output / Artifacts | Status | Quality Control & Verification Criteria |
| :--- | :--- | :--- | :--- | :--- | :---: | :--- |
| **M2.1** | **Extract Per-Subject Cue Counts** | Parse master `annotations.json` to compute exact frame-level occurrence frequencies for each of the 4 target warning cues per subject ($S_1, \dots, S_{14}$). If a frame contains multiple cues, count once toward each active cue. | [`dataset/annotations.json`](file:///c:/Dev/repos/Public%20repos/DMS-Eval/dataset/annotations.json) | Per-subject cue distribution matrix (CSV / JSON) | 🧊 Frozen | Frame counts tabulated (not raw box counts). All 14 subjects profiled. |
| **M2.2** | **Formalize 8/3/3 Subject Assignment Method** | Implement a deterministic combinatorial optimization / search algorithm (e.g., exhaustive evaluation of all $\binom{14}{8} \times \binom{6}{3} = 3003 \times 20 = 60,060$ valid 8/3/3 subject partitions) that minimizes cue distribution divergence (e.g., normalized Kullback-Leibler divergence or Chi-squared distance) relative to the ~57.1% / 21.4% / 21.4% split ratio. | Per-subject cue matrix; `scripts/balance_splits.py` | Partition scoring logs & optimal subject candidate set | ⚠️ Resolve Later | Algorithm guarantees all 4 target cues are present in Train, Val, and Test splits. Deterministic, fully reproducible selection. |
