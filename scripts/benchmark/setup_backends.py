...
def _patch_ultralytics_trainer_source(source: str) -> str:
    ...
    replacements = {
        '                    self.accumulate = max(1, int(np.interp(ni, xi, [1, self.args.nbs / self.batch_size]).round()))':
            '                    # DMS-Eval: keep the shared four-step update cadence during LR/momentum warm-up.\n'
            '                    self.accumulate = max(round(self.args.nbs / self.batch_size), 1)',
        '                    # Backward\n                    self.scaler.scale(self.loss).backward()':
            '                    # DMS-Eval: sample-normalize the final incomplete accumulation window.\n'
    ...
    for old, new in replacements.items():
        if old not in patched:
            raise ProtocolError("Pinned Ultralytics trainer does not match the frozen patch anchors")
...
def ensure_ultralytics(install: bool) -> dict[str, str]:
    ...
    if actual_version != spec["version"]:
        raise ProtocolError(f"Ultralytics is {actual_version}, expected {spec['version']}")
    ...
    if "# DMS-Eval: keep the shared four-step update cadence" not in trainer_path.read_text(encoding="utf-8"):
...
def main() -> int:
    ...
    report = {"ultralytics": ensure_ultralytics(args.install), "dfine_checkout": ensure_dfine(args.install), "weights": {}}
