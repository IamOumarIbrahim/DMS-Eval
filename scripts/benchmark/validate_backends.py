...
from scripts.benchmark.setup_backends import ensure_dfine, ensure_ultralytics, ensure_weight


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--synthetic", action="store_true", help="Run one FP32-input CUDA AMP 1x3x640x640 forward per adapter")
    parser.add_argument("--device", default="cuda:0")
    args = parser.parse_args()
    specs = load_backends()
    report = {"ultralytics": ensure_ultralytics(False), "dfine_checkout": ensure_dfine(False), "models": {}}
    models = {
