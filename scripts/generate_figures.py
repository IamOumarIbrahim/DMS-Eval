...
def main() -> int:
    ...
    for row in rows:
        axis.scatter(row["p50_ms"], row["map_50_95"], s=75)
        axis.annotate(row["model_id"], (row["p50_ms"], row["map_50_95"]), xytext=(5, 5), textcoords="offset points")
    axis.set_xlabel("Model-only p50 latency (ms), batch 1 FP16")
