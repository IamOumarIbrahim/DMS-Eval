...
def latency_summary(latencies_ms: Iterable[float], *, timing: str, boundary: str) -> dict[str, Any]:
    latencies = np.asarray(list(latencies_ms), dtype=np.float64)
    if latencies.size == 0 or not np.all(np.isfinite(latencies)) or np.any(latencies <= 0):
...
class CudaForwardProfiler:
    ...
    def prepare(self, sample: torch.Tensor) -> None:
        if sample.shape != (1, 3, 640, 640) or sample.dtype != torch.float32 or sample.device.type != "cuda":
            raise ProtocolError("Profiler input must be CUDA FP32 with shape 1x3x640x640")
    ...
    def finish(self) -> dict[str, Any]:
        ...
        if self._pending_wall_start is not None or len(self.events) != len(self.end_to_end_ms):
...
@torch.inference_mode()
def model_flop_estimates(adapter: DetectorAdapter, sample: torch.Tensor | None = None) -> dict[str, Any]:
    ...
    except Exception as exc:  # Operator support varies by architecture and PyTorch build.
        estimates["torch_profiler"] = {
            "flops": None,
            "method": "torch.profiler with_flops operator sum",
            "status": "unavailable",
    ...
    estimates["interpretation"] = "Tool-dependent estimates; disagreement indicates operator-coverage uncertainty."
    return estimates
