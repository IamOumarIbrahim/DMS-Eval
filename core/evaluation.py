...
def validate_predictions(predictions: Iterable[dict[str, Any]], valid_image_ids: set[int] | None = None) -> list[dict[str, Any]]:
    ...
    for index, prediction in enumerate(predictions):
        ...
        if valid_image_ids is not None and image_id not in valid_image_ids:
            raise ProtocolError(f"Prediction {index} references image {image_id} outside the selected split")
        if category_id not in {1, 2, 3, 4}:
            raise ProtocolError(f"Prediction {index} has invalid category {category_id}")
        ...
        normalized.append({"image_id": image_id, "category_id": category_id, "bbox": bbox, "score": score})
    return normalized


def split_ground_truth(master: dict[str, Any], subjects: Iterable[str]) -> dict[str, Any]:
    """Return a COCO object filtered by subject while preserving master image order."""
    subject_set = set(subjects)
    images = [image for image in master["images"] if Path(image["file_name"]).parts[1] in subject_set]
    image_ids = {int(image["id"]) for image in images}
    annotations = [annotation for annotation in master["annotations"] if int(annotation["image_id"]) in image_ids]
    result = {key: value for key, value in master.items() if key not in {"images", "annotations"}}
    result["images"] = images
    result["annotations"] = annotations
    return result


def load_ground_truth(annotations_path: str | Path, split: str, splits_path: str | Path) -> dict[str, Any]:
    ...
    for image in ground_truth["images"]:
        ...
        for detection in detections:
            ...
            for gt_index, annotation in enumerate(ground_truths):
                if gt_index in matched or int(annotation["category_id"]) != detection["category_id"]:
                    continue
...
def select_threshold_candidate(candidates: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    return max(candidates, key=lambda item: (item["micro_f1"], item["precision"], item["threshold"]))


def calibrate_threshold(ground_truth: dict[str, Any], predictions: Iterable[dict[str, Any]]) -> dict[str, Any]:
    """Validation-only threshold search: F1, then precision, then higher threshold."""
    predictions = list(predictions)
    candidates = [operating_point_metrics(ground_truth, predictions, threshold) for threshold in threshold_grid()]
...
def coco_metrics(ground_truth: dict[str, Any], predictions: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        ...
        if predictions:
            ...
            coco_dt = COCO()
            coco_dt.dataset = {"images": ground_truth["images"], "annotations": [], "categories": ground_truth["categories"]}
            coco_dt.createIndex()
    ...
    names = {int(item["id"]): item["name"] for item in ground_truth["categories"]}
    for category_index, category_id in enumerate(evaluator.params.catIds):
        values = precision[:, :, category_index, 0, -1]
        valid = values[values > -1]
        per_class[names[category_id]] = float(np.mean(valid)) if valid.size else float("nan")
    return {
...
def read_prediction_envelope(path: str | Path, required_split: str | None = None) -> dict[str, Any]:
    ...
    if required_split is not None and envelope.get("split") != required_split:
        raise ProtocolError(f"Expected {required_split} predictions, got {envelope.get('split')}")
